...
init_install() {
  ...
  echo "Installing genome for:" > /dev/stderr 
  echo "species: $SPECIES" > /dev/stderr 
  echo "assembly: $ASSEMBLY" > /dev/stderr 
  echo "source: $SOURCE" > /dev/stderr 
  echo "in: $INSTALL_DIR" > /dev/stderr 
  echo
...
download_path() {
  ...
  echo $DOWNLOAD_DIR/`echo $URL | perl -pe 's,^[^:]*://,,'` > /dev/stderr 
}
...
set_urls() {
  ...
  then
    ...
    then
      echo "Primary assembly not available for $SPECIES, use toplevel assembly instead" > /dev/stderr 
    ...
    then
      echo "VCF not available for $SPECIES" > /dev/stderr 
      VCF_URL=
    fi
    ...
    then
      echo "VCF tabix index not available for $SPECIES" > /dev/stderr 
      VCF_TBI_URL=
    fi
...
cmd_or_job() {
  ...
  then
    echo
    echo "Submitting $JOB_PREFIX as job..." > /dev/stderr 
    echo
    if [[ $HOST == "ip03" ]]; then
      ...
      echo -e "#! /bin/bash \n ${!CMD}" | sbatch --mail-type END,FAIL --job-name $JOB_PREFIX.$GENOME_FASTA --mail-user $JOB_MAIL -A $RAP_ID -D $INSTALL_DIR -e $LOG_DIR/${JOB_PREFIX}_$TIMESTAMP.log -o $LOG_DIR/${JOB_PREFIX}_$TIMESTAMP.log --mem-per-cpu=30000m --time=24:00:00 --cpus-per-task=$CORES
    fi
  else
    echo
...
create_picard_index() {
  ...
  then
    echo
    echo "Creating genome Picard sequence dictionary..." > /dev/stderr 
    echo
    ...
    echo
    echo "Genome Picard sequence dictionary up to date... skipping" > /dev/stderr 
    echo
  fi
}
...
create_samtools_index() {
  ...
  then
    echo
    echo "Creating genome SAMtools FASTA index..." > /dev/stderr 
    echo
    module load $module_samtools
    samtools faidx $GENOME_DIR/$GENOME_FASTA > $LOG_DIR/samtools_$TIMESTAMP.log 2>&1
  else
    echo
    echo "Genome SAMtools FASTA index up to date... skipping" > /dev/stderr 
    echo
  fi
}
...
create_bwa_index() {
  ...
  then
    echo
    echo "Creating genome BWA index..." > /dev/stderr 
    echo
    BWA_CMD="\
mkdir -p $INDEX_DIR && \
...
chmod -R ug+rwX,o+rX $INDEX_DIR $LOG"
    ...
    echo
    echo "Genome BWA index up to date... skipping" > /dev/stderr 
    echo
  fi
}
...
create_bowtie_tophat_index() {
  ...
  then
    echo
    echo "Creating genome Bowtie index and gtf TopHat index..." > /dev/stderr 
    echo
    BOWTIE_CMD="\
...
chmod -R ug+rwX,o+rX $BOWTIE_INDEX_DIR $LOG $ERR"
  cmd_or_job BOWTIE_CMD 4
    ...
    echo
    echo "Genome Bowtie index and gtf TopHat index up to date... skipping" > /dev/stderr 
    echo
  fi
}
...
create_bowtie2_tophat_index() {
  ...
  then
    echo
    echo "Creating genome Bowtie 2 index and gtf TopHat index..." > /dev/stderr 
    echo
    BOWTIE2_TOPHAT_CMD="\
...
chmod -R ug+rwX,o+rX \$TOPHAT_INDEX_DIR $LOG $ERR"
  cmd_or_job BOWTIE2_TOPHAT_CMD 4
    ...
    echo
    echo "Genome Bowtie 2 index and gtf TopHat index up to date... skipping" > /dev/stderr 
    echo
  fi
}
...
create_star_index() {
  ...
  do
    ...
    then
      echo
      echo "Creating STAR index with sjdbOverhang $sjdbOverhang..." > /dev/stderr 
      echo
      STAR_CMD="\
mkdir -p $INDEX_DIR && \
...
chmod -R ug+rwX,o+rX $INDEX_DIR \$LOG \$ERR"
      ...
      echo
      echo "STAR index with sjdbOverhang $sjdbOverhang up to date... skipping" > /dev/stderr 
      echo
    fi
  done
}
...
create_ncrna_bwa_index() {
  ...
  then
    echo
    echo "Creating ncRNA BWA index..." > /dev/stderr 
    echo
    mkdir -p $INDEX_DIR
    ...
    echo
    echo "ncRNA BWA index up to date... skipping" > /dev/stderr 
    echo
  fi
}
...
create_gene_annotations() {
  ...
  then
    echo
    echo "Creating gene ID, symbol, length annotations from GTF..." > /dev/stderr 
    echo
    cd $ANNOTATIONS_DIR
    module load $module_R
    module load $module_mugqic_R_packages
...
EOF
    ...
    echo
    echo "Gene ID, symbol, length annotations from GTF up to date... skipping" > /dev/stderr 
    echo
  fi
}
...
create_gene_annotations_flat() {
  ...
  then
    echo
    echo "Creating gene refFlat file from GTF..." > /dev/stderr 
    echo
    cd $ANNOTATIONS_DIR
...
#    gtfToGenePred -genePredExt -geneNameAsName2 ${ANNOTATION_PREFIX}.gtf ${ANNOTATION_PREFIX}.refFlat.tmp.txt
    ...
    echo
    echo "Creating gene refFlat file from GTF up to date... skipping" > /dev/stderr 
    echo
  fi
}
...
create_go_annotations() {
  ...
  then
    ...
    then
      echo
      echo "Retrieving GO annotations using BioMart..." > /dev/stderr 
      echo
      module load $module_R
      module load $module_mugqic_R_packages
      R --no-restore --no-save<<EOF
...
get_vcf_dbsnp() {
  ...
  then
    ...
    then
      echo
      echo "Retrieving dbSNP latest version from VCF..." > /dev/stderr 
      echo
 ...
 echo "Copying files" > /dev/stderr 	
  ...
  then
    ...
    if ! is_up2date $ANNOTATIONS_DIR/$CDNA ; then gunzip -c `download_path $CDNA_URL` > $ANNOTATIONS_DIR/$CDNA ; fi
     ...
     echo "Annotations uptodate" > /dev/stderr 
    ...
    then
      ...
      then
        ...
        grep -Pzoi ">.*rRNA[^>]*" $ANNOTATIONS_DIR/$NCRNA | grep -av "^$" > $ANNOTATIONS_DIR/$RRNA
	echo "Checking twice" > /dev/stderr 
      fi
    fi

    echo "Created rRNA Fasta" > /dev/stderr 
    if [ ! -z "${VCF_URL:-}" ]
    then
   ...
   echo "TABIX" > /dev/stderr 	
   #get_vcf_dbsnp
  else
    if ! is_up2date $ANNOTATIONS_DIR/$GTF
    then
      echo "Could not find $ANNOTATIONS_DIR/$GTF...\nyou might consider to manually download a gtf file from UCSC table browser (http://genome.ucsc.edu/cgi-bin/hgTables)" > /dev/stderr 
    else
      TRANSCRIPT_ID_GTF=$ANNOTATIONS_DIR/${GTF/.gtf/.transcript_id.gtf}
      if ! is_up2date $TRANSCRIPT_ID_GTF ; then grep -P "(^#|transcript_id)" $ANNOTATIONS_DIR/$GTF > $TRANSCRIPT_ID_GTF ; fi
    fi
    ...
    then
      echo "Could not find $ANNOTATIONS_DIR/$NCRNA...\nyou might consider to use the ncrna.fa file from Ensembl... " > /dev/stderr 
    else
      if [ $(grep -q -i "rRNA" $ANNOTATIONS_DIR/$NCRNA)$? == 0 ]
      then
        if ! is_up2date $ANNOTATIONS_DIR/$RRNA
...
install_genome() {
  ...
  set_urls
  #download_urls
 echo "Downloaded Files" > /dev/stderr 
  copy_files
  echo "copied Files" > /dev/stderr 
  build_files
  echo "build indexes" > /dev/stderr 
  create_genome_ini_file
