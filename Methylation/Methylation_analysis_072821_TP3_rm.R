...
files <- list.files(path=path,pattern="input",full.names=T)
files <- files[-grep("3DNA",basename(files))]

files
bismark_files <- as.list(files)
sample_id <- gsub("\\.readset_sorted.dedup.map.input","",basename(files))
out <- "Differential_Methylation_3rd_iteration_Timepoint3_removed"
...
targets <- targets[targets$timepoint != 3,]
comparisons <- list(
...
load(file.path(out,"objects_for_dm.rda"))

Diffs <- map2(meth_objs_for_dm,names(meth_objs_for_dm),function(x,n) {
	diff_stats <- calculateDiffMeth(x,mc.cores=8)
...
meth <- regionCounts(meth,x,save.db=F)
	...
	## better pca plot
        ...
        labs(x=paste0("PC1: ",var[1],"% variance"),y=paste0("PC2: ",var[2],"% variance")) +
        scale_shape_manual(values=c(16,2)) +
        #geom_text_repel(data=filter(pca_plot_df,PC2<(-100)|PC1>200),aes(label=Sample)) +
        theme_bw()


        ggsave(p,file=file.path(out,paste0("PCA_",n,".png")))
...
#
#
#ggsave(p,file="PCA_promters.png")
#
#### exons
